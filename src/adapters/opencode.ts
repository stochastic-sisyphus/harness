...
const OPENCODE_BIN = `${process.env.HOME}/.local/share/mise/shims/opencode`
...
export class OpenCodeAdapter implements ExecutorAdapter {
  ...
  async run(input: RunInput): Promise<RunHandle> {
    ...
    // Resume a prior session if we have an attempt with a sessionId
    const priorSessionId = this.extractSessionId(input)
    if (priorSessionId) {
      args.splice(1, 0, "--session", priorSessionId)
    }
  ...
  /**
   * Extract a session ID from prior attempts if available.
   * OpenCode stores sessions in SQLite, so we can resume via --session.
   */
  private extractSessionId(input: RunInput): string | undefined {
    // Look for a sessionId stashed in prior attempt outputs
    for (const attempt of input.context.priorAttempts) {
      const runId = attempt.runId
      if (runId) {
        // If a prior run stored a sessionId in its outputs, the orchestrator
        // would pass it through context. For now, we don't have that plumbing,
        // so return undefined and let opencode start a fresh session.
      }
    }
    return undefined
  }
  ...
  private tryParseJson(raw: string): Record<string, any> | null {
    try {
      ...
      // Try to find a JSON object in the output (opencode may prefix with non-JSON)
